 ...
 * @author carlos
 */
public class RungerKutta4 {

    static String expressao;
    float a, b, y0, x0 = 0, h;
    static double precisao;
    static int iteracoes;
    static String directoryName = "Metodo Runge Kutta 4";
    static ArrayList<String> resultado = new ArrayList<>();

    public static void main(String[] args) {
        RungerKutta4 rk4 = new RungerKutta4();
    }
    ...
    private double calculaMetodo(double xo, double yo, double h) {
        int it = 0; //Guarda a quantidade de iterações  
        String r;// guarda os resultados das iterações

        String titulo = "[MÉTODO RUNGE KUTTA 4]\n";
        ...
        while (xo < b) {
            ...
            /* calcula as constantes de Runger Kutta e tira a media ponderada*/
            double k1 = h * calculaFuncao(xo, yo);
            double k2 = h * calculaFuncao(xo + (h / 2), yo + (k1 / 2));
            double k3 = h * calculaFuncao(xo + (h / 2), yo + (k2 / 2));
            double k4 = h * calculaFuncao(xo + h, yo + k3);
            double y1 = yo + (1 / 6) * (k1 + 2 * k2 + 2 * k3 + k4);
            r = (it + "\t" + xo + "\t" + y1);
            xo = xo + h;
            yo = y1;
